 ...
 */
#if USE_SENSORLESS_HOMING || USE_COLLISION_DETECTION
  void GcodeSuite::M914() {

    bool report = true;
    const uint8_t index = parser.byteval('I');
    LOOP_XYZE(i) if (parser.seen(axis_codes[i])) {
      const int16_t value = parser.intval(axis_codes[i]) ;
      report = false;
      switch (i) {
        case X_AXIS:
          #if X_SENSORLESS
        ...
        case E_AXIS: {
          ...
          switch (target_extruder) {
            #if E0_SENSORLESS
                case 0: stepperE0.homing_threshold(value); break;
              #if E1_SENSORLESS
                case 1: stepperE1.homing_threshold(value); break;
              #endif
              #if E2_SENSORLESS
                case 2: stepperE2.homing_threshold(value); break;
              #endif
              #if E3_SENSORLESS
                case 3: stepperE3.homing_threshold(value); break;
              #endif
              #if E4_SENSORLESS
                case 4: stepperE4.homing_threshold(value); break;
              #endif
              #if E5_SENSORLESS
                case 5: stepperE5.homing_threshold(value); break;
              #endif
              #if E6_SENSORLESS
                case 6: stepperE6.homing_threshold(value); break;
              #endif
              #if E7_SENSORLESS
                case 7: stepperE7.homing_threshold(value); break;
              #endif
            #endif // E0_SENSORLESS
          }
        } break; // E_AXIS
    ...
    if (report) {
        ...
        #if E1_SENSORLESS
          tmc_print_sgt(stepperE1);
        #endif
        #if E2_SENSORLESS
          tmc_print_sgt(stepperE2);
        #endif
        #if E3_SENSORLESS
          tmc_print_sgt(stepperE3);
        #endif
        #if E4_SENSORLESS
          tmc_print_sgt(stepperE4);
        #endif
        #if E5_SENSORLESS
          tmc_print_sgt(stepperE5);
        #endif
        #if E6_SENSORLESS
          tmc_print_sgt(stepperE6);
        #endif
        #if E7_SENSORLESS
          tmc_print_sgt(stepperE7);
        #endif
      #endif // E0_SENSORLESS
    }
  }
...
/**
 * M915: Set StallGuard feedrate threshold.
 */
   ...
   void GcodeSuite::M915() {
    ...
    const uint8_t index = parser.byteval('I');
    LOOP_XYZE(i) if (parser.seen(axis_codes[i])) {
      const uint32_t value = parser.ulongval(axis_codes[i]);
      report = false;
        switch (i) {
        case X_AXIS:
          #if X_SENSORLESS
            if (index < 2) stepperX.velocity_threshold(value);
            #if X2_SENSORLESS
              if (!(index & 1)) stepperX2.velocity_threshold(value);
            #endif
          #endif // X_SENSORLESS
          break; // X_AXIS
        ...
        case Y_AXIS:
          #if Y_SENSORLESS
            if (index < 2) stepperY.velocity_threshold(value);
            #if Y2_SENSORLESS
              if (!(index & 1)) stepperY2.velocity_threshold(value);
            #endif
          #endif // Y_SENSORLESS
          break; // Y_AXIS
        ...
        case Z_AXIS:
          #if Z_SENSORLESS
            if (index < 2) stepperZ.velocity_threshold(value);
            #if Z2_SENSORLESS
              if (index == 0 || index == 2) stepperZ2.velocity_threshold(value);
            #endif
            #if Z3_SENSORLESS
              if (index == 0 || index == 3) stepperZ3.velocity_threshold(value);
            #endif
            #if Z4_SENSORLESS
              if (index == 0 || index == 4) stepperZ4.velocity_threshold(value);
            #endif
          #endif // Z_SENSORLESS
          break; // Z_AXIS
        ...
        case E_AXIS: {
          ...
          switch (target_extruder) {
            #if E0_SENSORLESS
              case 0: stepperE0.velocity_threshold(value); break;
              #if E1_SENSORLESS
                case 1: stepperE1.velocity_threshold(value); break;
              #endif
              #if E2_SENSORLESS
                case 2: stepperE2.velocity_threshold(value); break;
              #endif
              #if E3_SENSORLESS
                case 3: stepperE3.velocity_threshold(value); break;
              #endif
              #if E4_SENSORLESS
                case 4: stepperE4.velocity_threshold(value); break;
              #endif
              #if E5_SENSORLESS
                case 5: stepperE5.velocity_threshold(value); break;
              #endif
              #if E6_SENSORLESS
                case 6: stepperE6.velocity_threshold(value); break;
              #endif
              #if E7_SENSORLESS
                case 7: stepperE7.velocity_threshold(value); break;
              #endif
            #endif // E0_SENSORLESS
          }
        } break; // E_AXIS
    ...
    if (report) {
      #if X_SENSORLESS
        tmc_print_sgv(stepperX);
        #if X2_SENSORLESS
          tmc_print_sgv(stepperX2);
        #endif
      #endif // X_SENSORLESS
      
      #if Y_SENSORLESS
        tmc_print_sgv(stepperY);
        #if Y2_SENSORLESS
          tmc_print_sgv(stepperY2);
        #endif
      #endif // Y_SENSORLESS

      #if Z_SENSORLESS
        tmc_print_sgv(stepperZ);
        #if Z2_SENSORLESS
          tmc_print_sgv(stepperZ2);
        #endif
        #if Z3_SENSORLESS
          tmc_print_sgv(stepperZ3);
        #endif
        #if Z4_SENSORLESS
          tmc_print_sgv(stepperZ4);
        #endif
      #endif // Z_SENSORLESS

      #if E0_SENSORLESS
        tmc_print_sgv(stepperE0);
        #if E1_SENSORLESS
          tmc_print_sgv(stepperE1);
        #endif
        #if E2_SENSORLESS
          tmc_print_sgv(stepperE2);
        #endif
        #if E3_SENSORLESS
          tmc_print_sgv(stepperE3);
        #endif
        #if E4_SENSORLESS
          tmc_print_sgv(stepperE4);
        #endif
        #if E5_SENSORLESS
          tmc_print_sgv(stepperE5);
        #endif
        #if E6_SENSORLESS
          tmc_print_sgv(stepperE6);
        #endif
        #if E7_SENSORLESS
          tmc_print_sgv(stepperE7);
        #endif
      #endif // E0_SENSORLESS
    }
  }
